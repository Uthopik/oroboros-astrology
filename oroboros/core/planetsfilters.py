...
class PlanetsFilter(db.Object):
	...
	def _save_dict(self):
		...
		sql = """insert into _PlanetsFilters select ?, _idx, ? from Planets
			where name = ?;"""
		for plnt, val in list(self._dict_.items()):
	...
	def __setitem__(self, key, value=False):
		...
		:raise KeyError: not found
		"""
		for plt in list(self._dict_.keys()):
			if plt == key:
	...
	def values(self):
		...
		:rtype: list
		"""
		return list(self._dict_.values())
