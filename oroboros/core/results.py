...
class Data(object):
	...
	def __init__(self, res):
		...
		"""
		# Si pyswisseph devuelve ((lon, lat, dist, spd_lon, spd_lat, spd_dist), star_name)
		if isinstance(res, tuple) and len(res) > 0 and isinstance(res[0], (tuple, list)):
			res = res[0]
			
		if isinstance(res, (tuple, list)):
			self._longitude = float(res[0]) if len(res) > 0 else 0.0
			self._latitude = float(res[1]) if len(res) > 1 else 0.0
			self._distance = float(res[2]) if len(res) > 2 else 0.0
			self._lonspeed = float(res[3]) if len(res) > 3 else 0.0
			self._latspeed = float(res[4]) if len(res) > 4 else 0.0
			self._distspeed = float(res[5]) if len(res) > 5 else 0.0
		else:
			# Si res es un float/int escalar (por ejemplo, una cúspide de casa)
			self._longitude = float(res) if res is not None else 0.0
			self._latitude = 0.0
			self._distance = 0.0
			self._lonspeed = 0.0
			self._latspeed = 0.0
...
class PlanetData(Data):
	...
	def __repr__(self):
		return 'PlanetData(%s, (%s, %s, %s, %s, %s, %s))' % (
			repr(self._planet),
			repr(self._longitude),
			repr(self._latitude),
			repr(self._distance),
			repr(self._lonspeed),
			repr(self._latspeed),
			repr(self._distspeed)
		)
...
class PlanetDataList(list):
	...
	def sort_by_ranking(self, reverse=False):
		...
		:rtype: self
		"""
		self.sort(reverse=reverse,key=cmp_to_key(self._sort_by_ranking))
		return self
