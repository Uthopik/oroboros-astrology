...
class ChartDate(ChartFile):
	...
	def _calc_julday(self):
		...
		hour = utc_dt.hour + (utc_dt.minute / 60.0) + (utc_dt.second / 3600.0) + (utc_dt.microsecond / 3600000000.0)
		
		cal = swe.GREG_CAL if self.calendar in ('gregorian', None) else swe.JUL_CAL
		jd = float(swe.julday(utc_dt.year, utc_dt.month, utc_dt.day, hour, cal))
		...
		if self._utcoffset != None: # user-defined, below 1900
			...
			if self._zoneinfo == None: # assume it is utc
				...
				if self._dst != None: # user-defined, ambiguous
					loc_dt = tz.localize(self._datetime, is_dst=bool(self._dst))
	...
	def _calc_local_sidtime(self):
		...
		:rtype: time
		"""
		# Obtenemos el tiempo sidéreo en Greenwich para la fecha/hora exactas
		gst = self.sidtime  # Devuelve la hora sidérea en grados/horas decimales
		
		# Ajuste por longitud geográfica (15 grados = 1 hora)
		lon_deg = float(self._longitude.to_decimal()) if hasattr(self._longitude, 'to_decimal') else float(self._longitude)
		lst_hours = (gst + (lon_deg / 15.0)) % 24.0
		
		# Convertir a horas, minutos y segundos
		h = int(lst_hours)
		m = int((lst_hours - h) * 60.0)
		s = int((((lst_hours - h) * 60.0) - m) * 60.0)
		
		loc_sidt = time(hour=h, minute=m, second=s)
