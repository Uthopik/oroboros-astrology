...
class ChartPainter(QPainter):
	...
	def __init__(self, device, chart):
		...
		# =========================================================================
		# --- IMPRESIÓN DE DEPURACIÓN EXACTA ANTES DEL DIBUJADO ---
		# =========================================================================
		print("\n" + "=" * 70)
		print(f"[PAINTER FINAL CHECK] Objeto chart recibido: {chart}")
		print(f"[PAINTER FINAL CHECK] ID en memoria: {id(chart)}")
		
		sub_chart = chart[0] if hasattr(chart, '__getitem__') else chart
		print(f"[PAINTER FINAL CHECK] Sub-carta (idx=0) ID: {id(sub_chart)}")
		print(f"[PAINTER FINAL CHECK] Nombre: {getattr(sub_chart, 'name', 'N/A')}")
		print(f"[PAINTER FINAL CHECK] Datetime: {getattr(sub_chart, 'datetime', 'N/A')!r}")
		print(f"[PAINTER FINAL CHECK] Julian Day (JD): {getattr(sub_chart, 'julian_day', getattr(sub_chart, '_julian_day', 'N/A'))!r}")
		
		if hasattr(sub_chart, '_houses') and len(sub_chart._houses) > 0:
			house0_long = getattr(sub_chart._houses[0], '_longitude', 'N/A')
			print(f"[PAINTER FINAL CHECK] Cúspide Casa 1 (Asc): {house0_long}")
		else:
			print("[PAINTER FINAL CHECK] ATENCIÓN: _houses no está presente o está vacío.")
	...
	def draw_signs(self):
		...
		for i in range(12):
			...
			target = QRect(int(x), int(y), 16, 16)
	...
	def draw_cusps(self):
		...
		self.setPen(self._darkColor)
		self.drawLine(int(w), int(x), int(y), int(z))
		# draw other cusps
		...
		for i in othercusps:
			...
			self.drawLine(int(w), int(x), int(w1), int(x1))
			self.drawLine(int(y), int(z), int(y1), int(z1))
	...
	def draw_houses(self):
		...
		if self._chart[0].filter._hsys != 'G': ## traditional houses
			...
			for i in range(12):
				...
				target = QRect(int(x), int(y), 10, 10)
				self.drawImage(target, im, _iconRect)
		else: ## Gauquelin sectors
	...
	def draw_planets(self, num, lenvisible, plres):
		...
		for i, res in enumerate(plres):
			...
			try: ## planet
				...
				target = QRect(int(x), int(y), int(gw), int(gw))
				self.drawImage(target, im, _iconRect)
			except: ## midp
				...
				target = QRect(int(x), int(y), int(gw/1.6), int(gw/1.6))
