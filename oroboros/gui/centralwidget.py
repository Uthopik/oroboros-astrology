...
class CentralWidget(QTabWidget):
	...
	def drawOroboros(self):
		...
		target = QRect(
			(self.width()//2)-170, (self.height()//2)-170, 340, 340)
...
class ChartWheelWidget(QWidget):
	...
	def paintEvent(self, event):
		...
		cht = app.desktop.charts[self._idx][0]
		print(f"\n[DEBUG PAINT] Dibujando idx={self._idx}:")
		print(f"  - Nombre: {getattr(cht, '_name', 'N/A')}")
		print(f"  - Datetime: {getattr(cht, 'datetime', getattr(cht, '_datetime', 'N/A'))!r}")
		print(f"  - Julian Day (JD): {getattr(cht, 'julian_day', getattr(cht, 'jd', getattr(cht, '_jd', 'N/A')))!r}")
