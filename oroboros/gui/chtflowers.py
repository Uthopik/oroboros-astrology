...
class Bunch(list):
	...
	def inbunch(self, pos):
		diff = abs(swe.difdeg2n(pos, self.center))
		return diff <= self.orb()
	
	def mixbunch(self, other):
		diff = abs(swe.difdeg2n(self.center, other.center))
		return diff <= (self.orb() + other.orb())
	...
	def arrange(self):
		self.sort(key=cmp_to_key(self.cmp))
		ret = list()
