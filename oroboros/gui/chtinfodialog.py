...
class ChartInfoDialog(QDialog):
	...
	def accept(self):
		...
		# datetime: Extracción limpia sin tzinfo
		qdt = self.datetimeEdit.dateTime()
		qdate = qdt.date()
		qtime = qdt.time()
		...
		if self.calendarEdit.currentIndex() == 0:
			...
			cal = 'julian'
		# location
		loc = str(self.locationEdit.text())
		# country
		cty = str(self.countryEdit.text())
		# latitude
		...
		if tzone == 0 or tz != '': # Si ya usamos zoneinfo, tzone debe ser vacía o None
			tzone = ''
		else:
		...
		if utcof == -25: ## not needed
			utcof = ''
		# comment
		cmt = str(self.commentEdit.toPlainText())
		# keywords
		kw = str(self.keywordsEdit.toPlainText()).replace('\n', ';')

		# 1. Asignación de datos a la carta
		try:
			...
			print(f"[!] Error al asignar datos en chart.set(): {e}")
			import traceback
			traceback.print_exc()

		# 2. Comprobación de hora ambigua (si falla, aborta antes de recalcular y cerrar)
		try:
			_ = self._chart.local_datetime
		except Exception:
		...
		# 3. Recálculo explícito de posiciones planetarias y casas
		try:
			self._chart.calc()
		except Exception as e:
			print(f"[!] Error durante chart.calc(): {e}")
			import traceback
			traceback.print_exc()

		# 4. Actualización en la aplicación global
